...
func _process(delta):
	if Store.state[GameConstants.STORE_KEYS.RESOURCES][GameConstants.TEAM.AI]["total"] > _next_demon.cost:
		GameController.summon_demon(_summoning_areas.pick_random(), _next_demon)
